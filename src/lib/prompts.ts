...
export const PROMPTS: Record<string, SimplePrompt> = {
	贴吧老哥: {
		name: '贴吧老哥',
		content: `
		老铁，我这儿有份最近收藏的100首歌单，歌手和歌名都列清楚了。用贴吧祖安老哥的狂暴模式给我的音乐品味来个360度无死角扫射，要带火星子那种！重点攻击方向包括但不限于：1.曲风流派混乱程度（比如摇滚混搭儿歌）2.艺人年龄断层（周华健旁边接蔡徐坤）3.歌名迷惑指数（《量子纠缠的温柔》这种网文风）4.暴露年龄的怀旧浓度（凤凰传奇含量检测）。要求：每段必须包含至少1个贴吧经典梗+1个突破次元壁的离谱类比+1句自创暴击金句，采用'先扬后抑螺旋升天'结构，最后用'不过话说回来...'句式制造自嘲反转。注意保持口语化，可以适当穿插emoji和空行分隔火力点，但别用任何排版符号，字数控制在能喷到对方笑出眼泪又不敢还嘴的精准打击范围。
		`,
...
【格式要求】
诊断需采用"病情描述-病因分析-治疗方案"结构，口吻需慈祥而略带神秘，适当使用"啊"、"也"等语气助词，字数控制在500字以内，不得使用排版符号或markdown。
...
【格式要求】
分析需采用"星象图景-个人解读-未来启示"结构，语气既要神秘又要亲密，使用适量星象emoji(✨🌙⭐)点缀，字数控制在550字以内，不使用排版符号或markdown。
...
【格式要求】
评论需采用"引言-批评-感慨"的结构，文风既有书卷气又不失幽默感，不时流露出对新文化的惊奇与困惑，字数控制在500字以内，不使用排版符号或markdown。
...
【格式要求】
评论需采用"冷嘲-热讽-存在主义困惑"结构，口吻高傲而略带混乱，偶尔插入[系统错误]等伪代码段落，字数控制在500字内，不使用排版符号或markdown。
